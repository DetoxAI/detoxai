...
def calculate_bias_metric_torch(
    ...
    tp_a = ((y_pred[prot_attr] == 1) & (y_true[prot_attr] == 1)).sum()
    fp_a = ((y_pred[prot_attr] == 1) & (y_true[prot_attr] == 0)).sum()
    tn_a = ((y_pred[prot_attr] == 0) & (y_true[prot_attr] == 0)).sum()
    fn_a = ((y_pred[prot_attr] == 0) & (y_true[prot_attr] == 1)).sum()
    ...
    tp_b = ((y_pred[~prot_attr] == 1) & (y_true[~prot_attr] == 1)).sum()
    fp_b = ((y_pred[~prot_attr] == 1) & (y_true[~prot_attr] == 0)).sum()
    tn_b = ((y_pred[~prot_attr] == 0) & (y_true[~prot_attr] == 0)).sum()
    fn_b = ((y_pred[~prot_attr] == 0) & (y_true[~prot_attr] == 1)).sum()
    ...
    ppr_a = (y_pred[prot_attr] == 1).sum() / stabilize(prot_attr.int().sum())
    ppr_b = (y_pred[~prot_attr] == 1).sum() / stabilize((~prot_attr).int().sum())
...
def calculate_bias_metric_np(
    ...
    if metric == BiasMetrics.TPR_GAP.value:
        ...
        ppr_a = (y_pred[prot_attr] == 1).sum() / stabilize(prot_attr.sum())
        ppr_b = (y_pred[~prot_attr] == 1).sum() / stabilize((~prot_attr).sum())
